...
var analytics = (function () {
	...
	var address = function (proto) {
	  ...
	  function trait(a, b) {
	    return function () {
	      ...
	      return objCase(traits, 'address.' + a)
	        || objCase(traits, a)
	        || (b ? objCase(traits, 'address.' + b) : null)
	        || (b ? objCase(traits, b) : null)
	        || objCase(props, 'address.' + a)
	        || objCase(props, a)
	        || (b ? objCase(props, 'address.' + b) : null)
	        || (b ? objCase(props, b) : null);
	    };
	  }
	};
	...
	var typeComponent = function(val){
	...
	function traverse (input, strict) {
	  ...
	  if (componentType(input) == 'object') return object$1(input, strict);
	  if (componentType(input) == 'array') return array$1(input, strict);
	  return input;
	}
	...
	function object$1 (obj, strict) {
	  componentEach(obj, function (key, val) {
	    if (lib$2.is(val, strict)) {
	      obj[key] = lib$2.parse(val);
	    } else if (componentType(val) == 'object' || componentType(val) == 'array') {
	      traverse(val, strict);
	    }
	  });
	  return obj;
	...
	function array$1 (arr, strict) {
	  componentEach(arr, function (val, x) {
	    if (componentType(val) == 'object') {
	      traverse(val, strict);
	    } else if (lib$2.is(val, strict)) {
	      arr[x] = lib$2.parse(val);
	    }
	  });
	  return arr;
	}
	...
	function Facade(obj, opts) {
	  ...
	  if (opts.traverse) isodateTraverse(obj);
	  this.opts = opts;
	  this.obj = obj;
	}
	...
	Facade.multi = function (path) {
	  return function () {
	    ...
	    if (type$2(multi) === 'array') return multi;
	...
	Facade.one = function (path) {
	  return function () {
	    ...
	    if (one) return one;
	    var multi = this.proxy(path + 's');
	    if (type$2(multi) === 'array') return multi[0];
	  };
	};
	...
	Facade.prototype.device = function () {
	  ...
	  if (type$2(device) !== 'object') device = {};
	...
	Identify.prototype.avatar = function () {
	  var traits = this.traits();
	  return objCase(traits, 'avatar') || objCase(traits, 'photoUrl') || objCase(traits, 'avatarUrl');
	};
	...
	var isArrayLike$1 = function isArrayLike(val) {
	  return val != null && (isArray(val) || (val !== 'function' && isNumber$2(val.length)));
	};
	...
	var is_1$1 = createCommonjsModule(function (module) {
	 ...
	 * //=> [1, 4, 9]
	 */
	var map$1 = function map(iterator, collection) {
	...
	function normalize(msg, list) {
	  var lower = map_1(function(s) { return s.toLowerCase(); }, list);
	  ...
	  each_1(function(value, key) {
	    ...
	    if (!has$3.call(integrations, key)) integrations[key] = value;
	    delete opts[key];
	  }, opts);
	  ...
	  each_1(function(value, key) {
	    ...
	    if (has$3.call(integrations, key) && typeof providers[key] === 'boolean') return;
	  ...
	  if (!ret.eventId) {
	    ret.eventId = uuid_1.v4();
	  }
	  // debug('->', ret);
	  return ret;

	  function integration(name) {
	    return !!(includes_1(name, list) || name.toLowerCase() === 'all' || includes_1(name.toLowerCase(), lower));
	  }
	}
	...
	Memory.prototype.get = function(key) {
	  if (!has$5.call(this.store, key)) return;
	...
	Entity.prototype.identify = function(id, traits) {
	  traits = traits || {};
	  var current = this.id();
	  if (current === null || current === id) traits = extend_1(this.traits(), traits);
	  if (id) this.id(id);
	...
	Entity.prototype.save = function() {
	  ...
	  cookie_1.set(this._options.cookie.key, this.id());
	  store_1.set(this._options.localStorage.key, this.traits());
	  return true;
	};
	...
	Entity.prototype.load = function() {
	  this.id(cookie_1.get(this._options.cookie.key));
	  this.traits(store_1.get(this._options.localStorage.key));
	};
	...
	var Identify$1 = lib$3.Identify;
	var Page$1 = lib$3.Page;
	var Track$1 = lib$3.Track;
	...
	Analytics.prototype.init = Analytics.prototype.initialize = function(settings, options) {
	  ...
	  each_1(function(opts, name) {
	    var Integration = self.Integrations[name];
	    var integration = new Integration(clone_1(opts));
	...
	Analytics.prototype.identify = function(id, traits, options, fn) {
	  ...
	  if (is_1$1.fn(options)) fn = options, options = null;
	  if (is_1$1.fn(traits)) fn = traits, options = null, traits = null;
	  if (is_1$1.object(id)) options = traits, traits = id, id = user.id();
	...
	Analytics.prototype.track = function(event, properties, options, fn) {
	  ...
	  /* eslint-disable no-unused-expressions, no-sequences */
	  if (is_1$1.fn(options)) fn = options, options = null;
	  if (is_1$1.fn(properties)) fn = properties, options = null, properties = null;
	  /* eslint-enable no-unused-expressions, no-sequences */
	...
	Analytics.prototype.page = function(category, name, properties, options, fn) {
	  ...
	  /* eslint-disable no-unused-expressions, no-sequences */
	  if (is_1$1.fn(options)) fn = options, options = null;
	  if (is_1$1.fn(properties)) fn = properties, options = properties = null;
	  if (is_1$1.fn(name)) fn = name, options = properties = name = null;
	  if (componentType(category) === 'object') options = name, properties = category, name = category = null;
	  if (componentType(name) === 'object') options = properties, properties = name, name = null;
	  if (componentType(category) === 'string' && componentType(name) !== 'string') name = category, category = null;
	  /* eslint-enable no-unused-expressions, no-sequences */
	...
	// Stringifier
	var toString$3 = commonjsGlobal.JSON && typeof JSON.stringify === 'function' ? JSON.stringify : String;
	...
	var loadIframe = function loadIframe(options, fn){
	  ...
	  if (is_1$1.string(options)) options = { src : options };
	...
	exports.load = function (name, locals, callback) {
	  ...
	  if (!template) throw new Error(lib$7('template "%s" not defined.', name));
	...
	exports._wrapPage = function () {
	  ...
	  if (this._assumesPageview) return this.page = after_1(2, this.page); // eslint-disable-line no-return-assign
	};
	...
	function getMappingType(mapping) {
	  if (is_1$1.array(mapping)) {
	    return every_1(isMixed, mapping) ? 'mixed' : 'array';
	  }
	  if (is_1$1.object(mapping)) return 'map';
	  return 'unknown';
	}
	...
	function isMixed(item) {
	  if (!is_1$1.object(item)) return false;
	  if (!is_1$1.string(item.key)) return false;
	...
	function createIntegration(name) {
	  ...
	  function Integration(options) {
	    ...
	    this.options = defaults_1(clone_1(options) || {}, this.defaults);
	    this._queue = [];
	  ...
	  extend_1(Integration, statics);
	  extend_1(Integration.prototype, protos);
	...
	analytics$1.VERSION = '3.0.0';

	analytics$1.createIntegration = lib$8;

	/*
	 * Exports.
	 */

	var lib$9 = analytics$1;
